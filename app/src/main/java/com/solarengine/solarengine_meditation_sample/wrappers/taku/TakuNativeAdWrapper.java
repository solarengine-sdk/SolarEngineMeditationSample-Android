...
/**
 * Taku Native Ad Wrapper
 * Provides callback interception methods for Taku Native Ad listeners
 */
public class TakuNativeAdWrapper {
    ...
    public static ATAdRevenueListener buildATAdRevenueListener(Context context,
        ...
        LogUtils.i("TakuNativeAdWrapper.buildATAdRevenueListener() called");

        return new ATAdRevenueListener() {
            @Override
            public void onAdRevenuePaid(@NonNull ATAdInfo atAdInfo) {
                LogUtils.i("Taku Native Ad onAdRevenuePaid() callback received");
                
                // 调用SolarEngine埋点上报广告展示事件
                TakuSolarEngineTracker.trackAdImpression(context, atAdInfo, Native);
